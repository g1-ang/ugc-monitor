#!/usr/bin/env bash
# sync_package.sh
# 본체 repo의 최신 코드를 팀 배포용 패키지 폴더(ugc_monitor_v1_260424/수정금지/)로 복사합니다.
# 내정보/, 캠페인자료/, venv/ 같은 유저 고유 데이터는 건드리지 않아요.

set -e

SRC="$(cd "$(dirname "$0")" && pwd)"
DST="$SRC/../ugc_monitor_v1_260424/수정금지"
...
echo "  2. zip 재생성:      (cd ..; zip -rq ugc_monitor_v1_260424.zip ugc_monitor_v1_260424 -x '*.DS_Store' '*__pycache__*')"
