...
**폴더 안의 `.command` 파일을 순서대로 더블클릭하면 됩니다.**

### 1️⃣ 처음 1번만 — `1. 최초 설치.command` 더블클릭
→ 자동으로 터미널 창 열리며 Python 패키지 설치 (5~10분).

### 2️⃣ 매번 사용 — `2. 실행.command` 더블클릭
→ 자동으로 서버 시작 + 브라우저에 `http://localhost:8000` 오픈.

> ⚠️ `install.sh` / `start.sh` 는 더블클릭하면 텍스트 편집기가 열려요 (실행 X).
> **반드시 `1. 최초 설치.command` / `2. 실행.command` 를 사용하세요.**
